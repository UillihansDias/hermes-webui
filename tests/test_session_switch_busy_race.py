...
def test_loadSession_snapshots_live_turn_before_wiping_message_pane():
    ...
    wipe_pos = body.find("_msgInner.replaceChildren()")
    assert snap_pos != -1, "loadSession must snapshot the outgoing live turn before switching"
    assert wipe_pos != -1, "loadSession must clear the outgoing transcript on switch"
    assert snap_pos < wipe_pos, "snapshot must run before msgInner is cleared"
